...
def find_new_path(recv_data, shortestPath):
    ...
    while obstacle_pos < len(obstacle_list):
        ...
        image_enhancer.colourCell(
            img, obstacle_list[obstacle_pos], obstacle_list[obstacle_pos+1], 0)
        obstacle_pos = obstacle_pos + 2
        # cv2.imshow('colored' + str(obstacle_pos), img)

    new_shortestPath = task_1a.solveMaze(
        img, new_initial_point, final_point, no_cells_height, no_cells_width)
...
if __name__ == '__main__':
    ...
    img_dir_path = curr_dir_path + '/../task_1b_images/'
    ...
    try:
        ...
        if (sent_data.count('#') == 2) and (recv_data.count('@') == 2):
            print('\nSending %d bytes of data to server = %s' %
                  (len(sent_data), sent_data))
            print('\nReceived %d bytes of data from server = %s' %
                  (len(recv_data), recv_data))
        ...
        while '$' not in recv_data:
            ...
            try:
                obstacle_coord, new_shortestPath, new_initial_point, img = find_new_path(
                    recv_data, shortestPath)

                if len(new_shortestPath) > 2:
                    img = image_enhancer.highlightPath(
                        img, new_initial_point, final_point, new_shortestPath)

                    print('\nDynamic Obstacle found at = (%d,%d)' %
                          (obstacle_coord[0], obstacle_coord[1]))
                    print('\n--------------------------------------------')
                    print('\nNew Shortest Path = %s \n\nLength of new Path = %d' % (
                        new_shortestPath, len(new_shortestPath)))
            ...
            cv2.imshow('canvas0' + str(file_num) +
                       '_obstacle_' + str(obstacle_count), img)
            cv2.waitKey(0)

            shortestPath = new_shortestPath
            sent_data, recv_data = send_to_receive_from_server(
                sock, shortestPath)
