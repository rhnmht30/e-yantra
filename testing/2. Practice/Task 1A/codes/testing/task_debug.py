...
def findNeighbours(img,row,column):
    ...
    while(True):
        if img[i, i] == 0:
            ...
            break
        i += 1
    if img[int(cell - border * 2),int(cell)] == 0 or img[int(cell), int(cell - border * 2)] == 0:
        cell = cell + border
    ...
    if img[int(top), int(col_new)] != 0:
        neighbours.append([row - 1, column])
    if img[int(bottom), int(col_new)] != 0:
        neighbours.append([row + 1, column])
    if img[int(row_new), int(left)] != 0:
        neighbours.append([row, column - 1])
    if img[int(row_new), int(right)] != 0:
        neighbours.append([row, column + 1])
    ###################################################
    return neighbours
...
def numberMaze(graph, initial, new, l):
    ...
    if empty == 0:
        ...
        for i in range(int(len(graph[initial]))):
            if new[tuple(graph[initial][i])] == -1:
                n = next_num()
